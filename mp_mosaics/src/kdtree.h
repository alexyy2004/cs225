...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode* buildTree(int start, int end, int d, vector<Point<Dim>>& points);
    //KDTreeNode constructor_helper(const vector<Point<Dim>>& newPoints, int dimension, RandIter begin, RandIter end);
  ...
  private:
     ...
     */
    void findNeighborHelper(const Point<Dim>& target, KDTreeNode* root, int dim, double& bestDistance, const Point<Dim>*& bestPoint) const;
};
