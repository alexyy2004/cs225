                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.empty()) {
        std::cout << "single is empty" << std::endl;
        smallest = mergeQueue.front();
        mergeQueue.pop();
    } else if (mergeQueue.empty()) {
        std::cout << "merge is empty" << std::endl;
        smallest = singleQueue.front();
        singleQueue.pop();
    } else {
        std::cout << "NEITHER" << std::endl;
        TreeNode* single_first = singleQueue.front();
        TreeNode* merge_first = mergeQueue.front();
        if (single_first->freq.getFrequency() <= merge_first->freq.getFrequency()) {
            smallest = single_first;
            singleQueue.pop();
        } else {
            smallest = merge_first;
            mergeQueue.pop();
        }
    }
    return smallest;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (unsigned int i = 0; i < frequencies.size(); i++) {
        ...
        // std::cout << cur.getCharacter() << cur.getFrequency() << std::endl;
    ...
    // for (unsigned int i = 0; i < frequencies.size(); i++) {
    //     std::cout << singleQueue.front()->freq.getCharacter() << singleQueue.front()->freq.getFrequency() << std::endl;
    //     singleQueue.pop();
    // }
    // std::cout << "TEMP" << std::endl;
    ...
    while ((singleQueue.size() + mergeQueue.size()) > 1) {
        ...
        std::cout << "ablbalbla" << std::endl;
        std::cout << first->freq.getCharacter() << first->freq.getFrequency() << std::endl;
        TreeNode* second = removeSmallest(singleQueue, mergeQueue);
        std::cout << second->freq.getCharacter() << second->freq.getFrequency() << std::endl;
        ...
        //     mergeQueue.pop();
        // } else {
        //     first = mergeQueue.front();
        //     mergeQueue.pop();
        //     second = mergeQueue.front();
        //     mergeQueue.pop();
        // }
        Frequency merge_freq = Frequency(first->freq.getCharacter() + second->freq.getCharacter(), first->freq.getFrequency() + second->freq.getFrequency());
        TreeNode* merge_node = new TreeNode(merge_freq);
