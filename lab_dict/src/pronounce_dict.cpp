...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    map<std::string, std::vector<std::string>>::const_iterator temp1 = dict.find(w1);
    map<std::string, std::vector<std::string>>::const_iterator temp2 = dict.find(w2);
    if (temp1 == dict.end() || temp2 == dict.end()) {
        return false;
    } else {
